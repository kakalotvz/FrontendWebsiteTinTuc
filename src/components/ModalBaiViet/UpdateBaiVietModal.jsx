...
export default function UpdateBaiVietModal({
  open,
  setOpen,
  hienThiPost,
  setDataPostUpdate,
  ...
  const handleOk = async () => {
    try {
      ...
      let res = await apiFetchTongQuat("/bai-viet/update-bai-viet", {
        method: "PUT",
        body: payload,
      });
      if (res.data) {
      ...
      console.error(err);
      message.error("Có lỗi xảy ra khi sửa bài viết.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  // ====== Upload Adapter (ẢNH + VIDEO + AUDIO) ======
  ...
  useEffect(() => {
    ...
    form.setFieldsValue({
      ...
      tags,
      status,
      theLoai: dataPostUpdate?.theLoai._id,
    });
    ...
    // Upload cover
    setCoverUrl(anhBia || "");
    setCoverPublicId(anhBiaPublicId || "");
    setFileList(
      anhBia
        ? [
            {
              ...
              status: "done",
              url: anhBia,
              public_id: anhBiaPublicId || "",
            },
          ]
        : []
    );
  }, [open, dataPostUpdate, form]);
  ...
  function insertOembed1(editor, url) {
    ...
    // const html = `<figure class="media"><video controls src="${url}" style="max-width:100%;height:auto;border-radius:8px"></video></figure>`;
    const html = `
    <figure class="media">
      <video
        src="${url}"
        ...
        <Row gutter={[24, 16]}>
          ...
          <Col xs={24} md={10}>
            ...
            {/* <Form.Item
              ...
              tooltip="Bật để bài viết xuất hiện trên trang công khai"
            >
